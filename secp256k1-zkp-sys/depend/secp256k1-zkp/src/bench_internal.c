...
typedef struct {
    secp256k1_zkp_scalar scalar[2];
    secp256k1_zkp_fe fe[4];
    secp256k1_zkp_ge ge[2];
    secp256k1_zkp_gej gej[2];
...
void bench_setup(void* arg) {
    ...
    secp256k1_zkp_scalar_set_b32(&data->scalar[0], init[0], NULL);
    secp256k1_zkp_scalar_set_b32(&data->scalar[1], init[1], NULL);
    secp256k1_zkp_fe_set_b32(&data->fe[0], init[0]);
    secp256k1_zkp_fe_set_b32(&data->fe[1], init[1]);
    secp256k1_zkp_fe_set_b32(&data->fe[2], init[2]);
    secp256k1_zkp_fe_set_b32(&data->fe[3], init[3]);
    CHECK(secp256k1_zkp_ge_set_xo_var(&data->ge[0], &data->fe[0], 0));
    CHECK(secp256k1_zkp_ge_set_xo_var(&data->ge[1], &data->fe[1], 1));
    secp256k1_zkp_gej_set_ge(&data->gej[0], &data->ge[0]);
    secp256k1_zkp_gej_rescale(&data->gej[0], &data->fe[2]);
    secp256k1_zkp_gej_set_ge(&data->gej[1], &data->ge[1]);
    secp256k1_zkp_gej_rescale(&data->gej[1], &data->fe[3]);
...
void bench_scalar_add(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_negate(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_negate(&data->scalar[0], &data->scalar[0]);
    }
}
...
void bench_scalar_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_sqr(&data->scalar[0], &data->scalar[0]);
    }
}
...
void bench_scalar_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_mul(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
}
...
void bench_scalar_split(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_split_lambda(&data->scalar[0], &data->scalar[1], &data->scalar[0]);
        j += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_inverse(&data->scalar[0], &data->scalar[0]);
        j += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_scalar_inverse_var(&data->scalar[0], &data->scalar[0]);
        j += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
void bench_field_normalize(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_normalize(&data->fe[0]);
    }
}
...
void bench_field_normalize_weak(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_normalize_weak(&data->fe[0]);
    }
}
...
void bench_field_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_mul(&data->fe[0], &data->fe[0], &data->fe[1]);
    }
}
...
void bench_field_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_sqr(&data->fe[0], &data->fe[0]);
    }
}
...
void bench_field_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_inv(&data->fe[0], &data->fe[0]);
        secp256k1_zkp_fe_add(&data->fe[0], &data->fe[1]);
    }
}
...
void bench_field_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_fe_inv_var(&data->fe[0], &data->fe[0]);
        secp256k1_zkp_fe_add(&data->fe[0], &data->fe[1]);
    }
}
...
void bench_field_sqrt(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        t = data->fe[0];
        j += secp256k1_zkp_fe_sqrt(&data->fe[0], &t);
        secp256k1_zkp_fe_add(&data->fe[0], &data->fe[1]);
    }
    CHECK(j <= iters);
}
...
void bench_group_double_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_gej_double_var(&data->gej[0], &data->gej[0], NULL);
    }
}
...
void bench_group_add_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_gej_add_var(&data->gej[0], &data->gej[0], &data->gej[1], NULL);
    }
}
...
void bench_group_add_affine(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_gej_add_ge(&data->gej[0], &data->gej[0], &data->ge[1]);
    }
}
...
void bench_group_add_affine_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_zkp_gej_add_ge_var(&data->gej[0], &data->gej[0], &data->ge[1], NULL);
    }
}
...
void bench_group_jacobi_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += secp256k1_zkp_gej_has_quad_y_var(&data->gej[0]);
        /* Vary the Y and Z coordinates of the input (the X coordinate doesn't matter to
           secp256k1_zkp_gej_has_quad_y_var). Note that the resulting coordinates will
           ...
           curve). */
        secp256k1_zkp_fe_add(&data->gej[0].y, &data->fe[1]);
        secp256k1_zkp_fe_add(&data->gej[0].z, &data->fe[2]);
        secp256k1_zkp_fe_normalize_var(&data->gej[0].y);
        secp256k1_zkp_fe_normalize_var(&data->gej[0].z);
    }
    CHECK(j <= iters);
}
...
void bench_group_to_affine_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        secp256k1_zkp_ge_set_gej_var(&data->ge[1], &data->gej[0]);
        ...
        secp256k1_zkp_fe_add(&data->gej[0].x, &data->ge[1].y);
        secp256k1_zkp_fe_add(&data->gej[0].y, &data->fe[2]);
        secp256k1_zkp_fe_add(&data->gej[0].z, &data->ge[1].x);
        secp256k1_zkp_fe_normalize_var(&data->gej[0].x);
        secp256k1_zkp_fe_normalize_var(&data->gej[0].y);
        secp256k1_zkp_fe_normalize_var(&data->gej[0].z);
    }
}
...
void bench_ecmult_wnaf(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += secp256k1_zkp_ecmult_wnaf(data->wnaf, 256, &data->scalar[0], WINDOW_A);
        overflow += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(overflow >= 0);
...
void bench_wnaf_const(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += secp256k1_zkp_wnaf_const(data->wnaf, &data->scalar[0], WINDOW_A, 256);
        overflow += secp256k1_zkp_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(overflow >= 0);
    CHECK(bits <= 256*iters);
...
void bench_num_jacobi(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    secp256k1_zkp_num nx, na, norder;

    secp256k1_zkp_scalar_get_num(&nx, &data->scalar[0]);
    secp256k1_zkp_scalar_order_get_num(&norder);
    secp256k1_zkp_scalar_get_num(&na, &data->scalar[1]);

    for (i = 0; i < iters; i++) {
        j += secp256k1_zkp_num_jacobi(&nx, &norder);
        secp256k1_zkp_num_add(&nx, &nx, &na);
    }
    CHECK(j <= iters);
}
#endif

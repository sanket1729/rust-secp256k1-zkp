...
void nonce_function_bip340_bitflip(unsigned char **args, size_t n_flip, size_t n_bytes) {
    ...
    secp256k1_zkp_testrand_flip(args[n_flip], n_bytes);
    CHECK(nonce_function_bip340(nonces[1], args[0], args[1], args[2], args[3], args[4]) == 1);
    CHECK(secp256k1_zkp_memcmp_var(nonces[0], nonces[1], 32) != 0);
}

/* Tests for the equality of two sha256 structs. This function only produces a
 * correct result if an integer multiple of 64 many bytes have been written
 * into the hash functions. */
void test_sha256_eq(const secp256k1_zkp_sha256 *sha1, const secp256k1_zkp_sha256 *sha2) {
    /* Is buffer fully consumed? */
    CHECK((sha1->bytes & 0x3F) == 0);

    CHECK(sha1->bytes == sha2->bytes);
    CHECK(secp256k1_zkp_memcmp_var(sha1->s, sha2->s, sizeof(sha1->s)) == 0);
}
...
void run_nonce_function_bip340_tests(void) {
    ...
    secp256k1_zkp_sha256 sha_optimized;
     ...
     * secp256k1_zkp_nonce_function_bip340_sha256_tagged has the expected
     * state. */
    secp256k1_zkp_sha256_initialize_tagged(&sha, tag, sizeof(tag));
    secp256k1_zkp_nonce_function_bip340_sha256_tagged(&sha_optimized);
    test_sha256_eq(&sha, &sha_optimized);

   /* Check that hash initialized by
    * secp256k1_zkp_nonce_function_bip340_sha256_tagged_aux has the expected
    * state. */
    secp256k1_zkp_sha256_initialize_tagged(&sha, aux_tag, sizeof(aux_tag));
    secp256k1_zkp_nonce_function_bip340_sha256_tagged_aux(&sha_optimized);
    test_sha256_eq(&sha, &sha_optimized);

    secp256k1_zkp_testrand256(msg);
    secp256k1_zkp_testrand256(key);
    secp256k1_zkp_testrand256(pk);
    secp256k1_zkp_testrand256(aux_rand);
...
void test_schnorrsig_api(void) {
    ...
    unsigned char msg[32];
    secp256k1_zkp_keypair keypairs[3];
    secp256k1_zkp_keypair invalid_keypair = { 0 };
    secp256k1_zkp_xonly_pubkey pk[3];
    secp256k1_zkp_xonly_pubkey zero_pk;
    ...
    secp256k1_zkp_testrand256(sk1);
    secp256k1_zkp_testrand256(sk2);
    secp256k1_zkp_testrand256(sk3);
    secp256k1_zkp_testrand256(msg);
    CHECK(secp256k1_zkp_keypair_create(ctx, &keypairs[0], sk1) == 1);
    CHECK(secp256k1_zkp_keypair_create(ctx, &keypairs[1], sk2) == 1);
    CHECK(secp256k1_zkp_keypair_create(ctx, &keypairs[2], sk3) == 1);
    CHECK(secp256k1_zkp_keypair_xonly_pub(ctx, &pk[0], NULL, &keypairs[0]) == 1);
    CHECK(secp256k1_zkp_keypair_xonly_pub(ctx, &pk[1], NULL, &keypairs[1]) == 1);
    CHECK(secp256k1_zkp_keypair_xonly_pub(ctx, &pk[2], NULL, &keypairs[2]) == 1);
    memset(&zero_pk, 0, sizeof(zero_pk));

    /** main test body **/
    ecount = 0;
    CHECK(secp256k1_zkp_schnorrsig_sign(none, sig, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_zkp_schnorrsig_sign(vrfy, sig, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, sig, msg, &keypairs[0], NULL, NULL) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, NULL, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, sig, NULL, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 4);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, sig, msg, NULL, NULL, NULL) == 0);
    CHECK(ecount == 5);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, sig, msg, &invalid_keypair, NULL, NULL) == 0);
    CHECK(ecount == 6);
    ...
    CHECK(ecount == 5);
    CHECK(secp256k1_zkp_schnorrsig_verify(vrfy, sig, msg, &zero_pk) == 0);
    CHECK(ecount == 6);
...
/* Checks that hash initialized by secp256k1_zkp_schnorrsig_sha256_tagged has the
 * expected state. */
void test_schnorrsig_sha256_tagged(void) {
    ...
    secp256k1_zkp_sha256_initialize_tagged(&sha, (unsigned char *) tag, sizeof(tag));
    secp256k1_zkp_schnorrsig_sha256_tagged(&sha_optimized);
    test_sha256_eq(&sha, &sha_optimized);
}
...
void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized, unsigned char *aux_rand, const unsigned char *msg, const unsigned char *expected_sig) {
    ...
    secp256k1_zkp_xonly_pubkey pk, pk_expected;

    CHECK(secp256k1_zkp_keypair_create(ctx, &keypair, sk));
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, NULL, aux_rand));
    CHECK(secp256k1_zkp_memcmp_var(sig, expected_sig, 64) == 0);

    CHECK(secp256k1_zkp_xonly_pubkey_parse(ctx, &pk_expected, pk_serialized));
    CHECK(secp256k1_zkp_keypair_xonly_pub(ctx, &pk, NULL, &keypair));
    CHECK(secp256k1_zkp_memcmp_var(&pk, &pk_expected, sizeof(pk)) == 0);
...
void test_schnorrsig_bip_vectors_check_verify(const unsigned char *pk_serialized, const unsigned char *msg32, const unsigned char *sig, int expected) {
    ...
    CHECK(secp256k1_zkp_xonly_pubkey_parse(ctx, &pk, pk_serialized));
...
void test_schnorrsig_bip_vectors(void) {
    ...
    }
    {
        ...
        secp256k1_zkp_xonly_pubkey pk_parsed;
        /* No need to check the signature of the test vector as parsing the pubkey already fails */
        CHECK(!secp256k1_zkp_xonly_pubkey_parse(ctx, &pk_parsed, pk));
    }
    {
        /* Test vector 6 */
        const unsigned char pk[32] = {
...
void test_schnorrsig_sign(void) {
    ...
    CHECK(secp256k1_zkp_keypair_create(ctx, &keypair, sk));
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, NULL, NULL) == 1);

    /* Test different nonce functions */
    memset(sig, 1, sizeof(sig));
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_failing, NULL) == 0);
    CHECK(secp256k1_zkp_memcmp_var(sig, zeros64, sizeof(sig)) == 0);
    memset(&sig, 1, sizeof(sig));
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_0, NULL) == 0);
    CHECK(secp256k1_zkp_memcmp_var(sig, zeros64, sizeof(sig)) == 0);
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_overflowing, NULL) == 1);
    CHECK(secp256k1_zkp_memcmp_var(sig, zeros64, sizeof(sig)) != 0);
}
...
void test_schnorrsig_sign_verify(void) {
    ...
    for (i = 0; i < N_SIGS; i++) {
        secp256k1_zkp_testrand256(msg[i]);
        CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig[i], msg[i], &keypair, NULL, NULL));
...

    {
        ...
        size_t sig_idx = secp256k1_zkp_testrand_int(N_SIGS);
        size_t byte_idx = secp256k1_zkp_testrand_int(32);
        unsigned char xorbyte = secp256k1_zkp_testrand_int(254)+1;
        sig[sig_idx][byte_idx] ^= xorbyte;
        CHECK(!secp256k1_zkp_schnorrsig_verify(ctx, sig[sig_idx], msg[sig_idx], &pk));
        sig[sig_idx][byte_idx] ^= xorbyte;

        byte_idx = secp256k1_zkp_testrand_int(32);
    ...
    /* Test overflowing s */
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig[0], msg[0], &keypair, NULL, NULL));
    CHECK(secp256k1_zkp_schnorrsig_verify(ctx, sig[0], msg[0], &pk));
    ...
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig[0], msg[0], &keypair, NULL, NULL));
    CHECK(secp256k1_zkp_schnorrsig_verify(ctx, sig[0], msg[0], &pk));
    secp256k1_zkp_scalar_set_b32(&s, &sig[0][32], NULL);
    secp256k1_zkp_scalar_negate(&s, &s);
    secp256k1_zkp_scalar_get_b32(&sig[0][32], &s);
    CHECK(!secp256k1_zkp_schnorrsig_verify(ctx, sig[0], msg[0], &pk));
}
#undef N_SIGS
...
void test_schnorrsig_taproot(void) {
    ...
    CHECK(secp256k1_zkp_xonly_pubkey_serialize(ctx, tweak, &internal_pk) == 1);
    ...
    CHECK(secp256k1_zkp_xonly_pubkey_serialize(ctx, output_pk_bytes, &output_pk) == 1);

    /* Key spend */
    secp256k1_zkp_testrand256(msg);
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, sig, msg, &keypair, NULL, NULL) == 1);
    /* Verify key spend */
    CHECK(secp256k1_zkp_xonly_pubkey_parse(ctx, &output_pk, output_pk_bytes) == 1);
    CHECK(secp256k1_zkp_schnorrsig_verify(ctx, sig, msg, &output_pk) == 1);

    /* Script spend */
    CHECK(secp256k1_zkp_xonly_pubkey_serialize(ctx, internal_pk_bytes, &internal_pk) == 1);
    /* Verify script spend */
    CHECK(secp256k1_zkp_xonly_pubkey_parse(ctx, &internal_pk, internal_pk_bytes) == 1);
    CHECK(secp256k1_zkp_xonly_pubkey_tweak_add_check(ctx, output_pk_bytes, pk_parity, &internal_pk, tweak) == 1);
}

...
int secp256k1_zkp_surjectionproof_allocate_initialized(const secp256k1_zkp_context* ctx, secp256k1_zkp_surjectionproof** proof_out_p, size_t *input_index, const secp256k1_zkp_fixed_asset_tag* fixed_input_tags, const size_t n_input_tags, const size_t n_input_tags_to_use, const secp256k1_zkp_fixed_asset_tag* fixed_output_tag, const size_t n_max_iterations, const unsigned char *random_seed32) {
    int ret = 0;
    ...
    *proof_out_p = 0;
    ...
    proof = (secp256k1_zkp_surjectionproof*)checked_malloc(&ctx->error_callback, sizeof(secp256k1_zkp_surjectionproof));
    if (proof != NULL) {
        ret = secp256k1_zkp_surjectionproof_initialize(ctx, proof, input_index, fixed_input_tags, n_input_tags, n_input_tags_to_use, fixed_output_tag, n_max_iterations, random_seed32);
        if (ret) {
            *proof_out_p = proof;
        }
        else {
...
/* secp256k1_zkp_surjectionproof structure may also be allocated on the stack,
 * and initialized explicitly via secp256k1_zkp_surjectionproof_initialize().
...
void secp256k1_zkp_surjectionproof_destroy(secp256k1_zkp_surjectionproof* proof) {
    if (proof != NULL) {
...
int secp256k1_zkp_surjectionproof_generate(const secp256k1_zkp_context* ctx, secp256k1_zkp_surjectionproof* proof, const secp256k1_zkp_generator* ephemeral_input_tags, size_t n_ephemeral_input_tags, const secp256k1_zkp_generator* ephemeral_output_tag, size_t input_index, const unsigned char *input_blinding_key, const unsigned char *output_blinding_key) {
    ...
    secp256k1_zkp_gej ring_pubkeys[SECP256K1_SURJECTIONPROOF_MAX_USED_INPUTS];
    secp256k1_zkp_scalar borromean_s[SECP256K1_SURJECTIONPROOF_MAX_USED_INPUTS];
    ...
    if (secp256k1_zkp_surjection_compute_public_keys(ring_pubkeys, n_used_pubkeys, ephemeral_input_tags, n_total_pubkeys, proof->used_inputs, ephemeral_output_tag, input_index, &ring_input_index) == 0) {
        return 0;
    }
    ...
    secp256k1_zkp_surjection_genmessage(msg32, ephemeral_input_tags, n_total_pubkeys, ephemeral_output_tag);
...
int secp256k1_zkp_surjectionproof_verify(const secp256k1_zkp_context* ctx, const secp256k1_zkp_surjectionproof* proof, const secp256k1_zkp_generator* ephemeral_input_tags, size_t n_ephemeral_input_tags, const secp256k1_zkp_generator* ephemeral_output_tag) {
    ...
    if (secp256k1_zkp_surjection_compute_public_keys(ring_pubkeys, n_used_pubkeys, ephemeral_input_tags, n_total_pubkeys, proof->used_inputs, ephemeral_output_tag, 0, NULL) == 0) {
        return 0;
    }
